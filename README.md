...
A service worker, module/worker scripts, and the AudioWorklet all require a **secure context**, so the app must be served over `http://` (or `https://`) — opening `index.html` from `file://` will not work. From the project folder:
...
The signal at the **centre** of the captured band is demodulated, so tune the dongle directly onto the station. Pick **Mode** (`WFM` for broadcast FM, `NFM` for narrowband FM, `AM`, or `CW` for Morse), toggle **Audio**, and set **Volume**. The dashed amber line marks the demodulated point. Audio is resampled to the browser's audio rate
automatically.

### CW (Morse)

`CW` mode adds a BFO that turns a keyed carrier at band centre into an audible tone, followed by a narrow band-pass filter to pull weak signals out of the noise. Two knobs in **Settings → CW** control it: **Pitch** (the sidetone frequency, default 700 Hz) and **Filter width** (narrower = more selective, default 300 Hz).

To hear it with no hardware, set **Source** to *Simulated (CW)* — it transmits `CQ DE TAUASA K` in Morse at 18 WPM on a centre carrier, and selecting it switches the mode to CW automatically. Click **Connect** and **Audio**.
